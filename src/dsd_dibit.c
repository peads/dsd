...
static int digitize(dsd_opts *opts, dsd_state *state, int symbol) {
    ...
    if ((state->synctype == 6) || (state->synctype == 14) || (state->synctype == 18)) {
    ...
    } else if ((state->synctype == 1) || (state->synctype == 3) || (state->synctype == 5) || (state->synctype == 9) || (state->synctype == 11) || (state->synctype == 13) || (state->synctype == 17)) {
        //  1 -P25p1
        ...
        if (state->synctype == 1) {
            ...
            valid = estimate_symbol(state->rf_mod,
                    &(state->inv_p25_heuristics),
                    state->last_dibit,
                    symbol,
                    &dibit);
        }
