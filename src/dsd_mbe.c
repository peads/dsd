...
// TODO refactor this into two, or more functions, s.t. we're not passing NULLs to determine state
//  and let the compiler figure out inlining.
void processMbeFrame(dsd_opts *opts,
    ...
    if ((state->synctype == 0) || (state->synctype == 1)) {
        ...
        mbe_processImbe7200x4400Framef(state->audio_out_temp_buf,
                &state->errs,
                &state->errs2,
                state->err_str,
                imbe_fr,
        ...
        mbe_processImbe7100x4400Framef(state->audio_out_temp_buf,
                &state->errs,
                &state->errs2,
                state->err_str,
                imbe7100_fr,
        ...
        mbe_processAmbe3600x2450Framef(state->audio_out_temp_buf,
                &state->errs,
                &state->errs2,
                state->err_str,
                ambe_fr,
