...
void processDMRdata(dsd_opts *opts, dsd_state *state) {
    ...
    char sync[25];
    char syncdata[25];
    // TODO figure out why these were being set, but never used
//    char cachdata[13];
//    char cc[5];
    char bursttype[5];
...
//    dibit = *dibit_p;
    dibit_p++;
    // TODO fix this bug: i.e., never using the inverted bit.
    //  Probably, save to addr stored in dibit_p instead
//    if (opts->inverted_dmr == 1) {
//        dibit = (dibit ^ 2);
//    }
...
//    cc[3] = (1 & dibit) + 48;     // bit 0
    ...
    bursttype[0] = (char) ((1 & (dibit >> 1)) + 48);       // bit 1
    bursttype[1] = (char) ((1 & dibit) + 48);      // bit 0
    ...
    bursttype[2] = (char) ((1 & (dibit >> 1)) + 48);       // bit 1
    bursttype[3] = (char) ((1 & dibit) + 48);      // bit 0
    ...
    for (i = 0; i < 24; i++) {
        ...
        syncdata[i] = (char) dibit;
        sync[i] = (char) ((dibit | 1) + 48);
    }
    sync[24] = 0;
    syncdata[24] = 0;
